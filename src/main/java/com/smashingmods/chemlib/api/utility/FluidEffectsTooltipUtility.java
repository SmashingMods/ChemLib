...
public class FluidEffectsTooltipUtility {
    ...
    public static void addTooltipEffects(List<MobEffectInstance> pEffects, List<Component> pTooltips) {
        ...
        if (pEffects.isEmpty()) {
            pTooltips.add(new TextComponent(" "));
            pTooltips.add(new TranslatableComponent("chemlib.effect.on_hit").withStyle(ChatFormatting.UNDERLINE).append(":"));
            pTooltips.add(new TranslatableComponent("effect.none").withStyle(ChatFormatting.GRAY));
        } else {
            pTooltips.add(new TextComponent(" "));
            pTooltips.add(new TranslatableComponent("chemlib.effect.on_hit").withStyle(ChatFormatting.UNDERLINE).append(":"));
            for (MobEffectInstance effectInstance : pEffects) {
                ...
                if (effectInstance.getAmplifier() > 0 && effectInstance.getAmplifier() <= 20) {
                    ...
                    mutableComponent = new TranslatableComponent("potion.withDuration", mutableComponent, MobEffectUtil.formatDuration(effectInstance, 1.0F));
                }
        ...
        if (!attributeModifierPairList.isEmpty()) {
            for (Pair<Attribute, AttributeModifier> attributeModifierPair : attributeModifierPairList) {
                ...
                if (baseModifierAmount > 0.0D) {
                    pTooltips.add(new TranslatableComponent(String.format("attribute.modifier.plus.%s", attributeModifier.getOperation().toValue()),
                            ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(finalModiferAmount),
                            new TranslatableComponent(attributeModifierPair.getKey().getDescriptionId()))
                            .withStyle(ChatFormatting.BLUE));

                } else if (baseModifierAmount < 0.0D) {
                    finalModiferAmount *= -1.0D;
                    pTooltips.add(new TranslatableComponent(String.format("attribute.modifier.take.%s", attributeModifier.getOperation().toValue()),
                            ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(finalModiferAmount),
                            new TranslatableComponent(attributeModifierPair.getKey().getDescriptionId()))
