                        ...
                        if (metalType == MetalType.METAL) {
                            ...
                            if (!hasItem) {
                                ...
                                BlockRegistry.BLOCKS.register(String.format("%s_metal_block", elementName),
                                        () -> new ChemicalBlock(ResourceLocation.fromNamespaceAndPath(ChemLib.MODID, elementName),
                                                ChemicalBlockType.METAL, BlockRegistry.METAL_BLOCKS, BlockRegistry.METAL_PROPERTIES));
                                BlockRegistry.getRegistryObjectByName(String.format("%s_metal_block", elementName))
                                        .ifPresent(block -> ItemRegistry.fromChemicalBlock(block, new Item.Properties()));
                            }
                        }
                        ...
                        if (!hasFluid) {
                            ...
                            if (group == 18) {
                                BlockRegistry.BLOCKS.register(String.format("%s_lamp_block", elementName),
                                        () -> new LampBlock(ResourceLocation.fromNamespaceAndPath(ChemLib.MODID, elementName),
                                                ChemicalBlockType.LAMP, BlockRegistry.LAMP_BLOCKS, BlockRegistry.LAMP_PROPERTIES));
                                BlockRegistry.getRegistryObjectByName(String.format("%s_lamp_block", elementName))
                                        .ifPresent(block -> ItemRegistry.fromChemicalBlock(block, new Item.Properties()));
                            }
    ...
    public static List<MobEffectInstance> mobEffectsFactory(JsonObject object) {
        ...
        if (effects != null) {
            for (JsonElement effect : effects) {
                ...
                ResourceLocation rl = ResourceLocation.parse(effectLocation);
                BuiltInRegistries.MOB_EFFECT.getHolder(rl).ifPresent(holder ->
                        effectsList.add(new MobEffectInstance(holder, effectDuration, effectAmplifier)));
            }
        }
        return effectsList;
    }
