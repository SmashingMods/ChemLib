    ...
    public static void register(IEventBus pEventBus) {
        ...
        METALS_TAB = REGISTRY_TABS.register("metals_tab", () -> CreativeModeTab.builder()
                ...
                .displayItems((pParameters, pOutput) -> {
                    ...
                    ItemRegistry.getChemicalBlockItems().stream()
                            .filter(item -> ((ChemicalBlock) item.getBlock()).getBlockType().getSerializedName().equals("metal"))
        ...
        MISC_TAB = REGISTRY_TABS.register("misc_tab", () -> CreativeModeTab.builder()
                ...
                .displayItems((pParameters, pOutput) -> {
                    ItemRegistry.REGISTRY_BLOCK_ITEMS.getEntries().stream()
                            .map(DeferredHolder::get)
                            .filter(item -> item.getDescriptionId().contains("lamp_block"))
                            .forEach(pOutput::accept);
                    ItemRegistry.REGISTRY_MISC_ITEMS.getEntries().stream()
                            .map(DeferredHolder::get)
                            .forEach(pOutput::accept);
